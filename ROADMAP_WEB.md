...
> **Meta**: Lanzar misiones Bug Bounty desde web, sin terminal.
> **Tiempo estimado**: 3-5 horas, una sesión.
...
- [ ] `src/core/web/assets/login.html` — Página login (input token + botón)
- [ ] `src/core/web/assets.rs` — Servir `/login` sin auth previa

### Comportamiento:
- Operador pega token de `workspace/logs/dashboard.token`
- Guarda en `sessionStorage` del browser
