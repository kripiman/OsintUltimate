...
impl BountyExporter {
    ...
    fn compress_evidence(finding: &Finding) -> String {
        let raw = serde_json::to_string_pretty(&finding.evidence.data).unwrap_or_default();
        const MAX: usize = 4096; // Increased to 4KB for better raw HTTP data
        if raw.len() > MAX {
    ...
    fn poc_steps(finding: &Finding) -> String {
        ...
        if let Some(url) = finding.evidence.data.get("url").and_then(|v| v.as_str()) {
    ...
    fn impact(finding: &Finding) -> String {
        ...
        format!("An attacker can leverage this {:?} severity issue in the {:?} category to compromise the target application's security posture.", finding.severity, finding.category)
    }

    fn attack_scenario(finding: &Finding) -> String {
        if let Some(ai) = &finding.ai_analysis {
            if !ai.stealth_notes.is_empty() {
                return ai.stealth_notes.clone(); // In V14.1, stealth_notes is often used for the scenario flow
            }
        }
    ...
    fn recommendation(finding: &Finding) -> String {
        // Generic but technically sound fallback based on category
        match finding.category {
            _ if finding.title.to_lowercase().contains("xss") => 
            ...
            _ if finding.title.to_lowercase().contains("idor") || finding.title.to_lowercase().contains("bola") => 
                "Implement object-level authorization checks. Ensure the authenticated user has rights to access the requested resource ID.",
            _ if finding.title.to_lowercase().contains("ssrf") => 
    ...
    fn generate_common_blocks(f: &Finding) -> String {
        ...
        if let Some(url) = f.evidence.data.get("url").and_then(|v| v.as_str()) {
        ...
        // ## References
        if !f.references.is_empty() || !f.cwe.is_empty() {
