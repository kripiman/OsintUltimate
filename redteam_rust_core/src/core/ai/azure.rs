...
impl LlmClient for AzureOpenAIClient {
    async fn analyze(&self, config: crate::core::ai::traits::InferenceConfig<'_>) -> Result<AIAnalysis> {
        ...
        let text = res["choices"][0]["message"]["content"].as_str().context(format!("Azure response format error. Raw response: {:?}", raw_res))?;
    ...
    async fn decide_action(&self, config: crate::core::ai::traits::DecisionConfig<'_>) -> Result<Option<(String, serde_json::Value)>> {
        ...
        let raw_res = res.clone();
        let text = res["choices"][0]["message"]["content"].as_str().context(format!("Azure decision format error. Raw response: {:?}", raw_res))?;
