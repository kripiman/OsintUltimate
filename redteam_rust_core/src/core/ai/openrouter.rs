...
impl LlmClient for OpenRouterClient {
    async fn analyze(&self, config: crate::core::ai::traits::InferenceConfig<'_>) -> Result<AIAnalysis> {
        ...
        let client = self.base.get_client("openrouter.ai").await?;
        let res = client.post("https://openrouter.ai/api/v1/chat/completions")
            .header("Authorization", format!("Bearer {}", self.key))
            .header("HTTP-Referer", "https://redteam.lab")
            .header("X-Title", "OsintUltimate RedTeam Engine")
            .json(&json!({
                "model": self.model,
                "messages": [
        ...
        let text = res["choices"][0]["message"]["content"].as_str().context("OpenRouter response format error")?;
