...
impl BountySink {
    ...
    /// Mirrors McpServer::with_off_path_engine() pattern (V14.6).
    ...
    /// One report per finding (H1/Bugcrowd best practice: one vulnerability per report).
...
impl BugBountyDraftSink {
    ...
    /// If directory creation fails, it logs a warning but allows the sink to exist (noop mode).
