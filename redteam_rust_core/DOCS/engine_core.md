...
- **Enriquecimiento (Intel)**: Los hallazgos se mapean automáticamente a CVEs mediante un `CveCacheManager` global y se puntúan usando **CVSS v3.1 real** (vía `cvss` crate).
- **Fail-Closed Scope**: Validación obligatoria contra la política de alcance antes de procesar cualquier objetivo. Si el objetivo es ajeno al programa, se genera un hallazgo de auditoría y se detiene la ejecución para ese host.
...
- **MultiSink**: Distribuye los resultados a PostgreSQL, JSONL y W
